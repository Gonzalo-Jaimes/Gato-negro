...
// --- CONFIGURACI├ôN DE SUPABASE (LA NUBE) ---
...
console.log("Γÿü∩╕Å Conectado a la base de datos en Supabase");

// --- INTEGRACI├ôN DE TELEGRAM BOT (WEBHOOK) ---
const bot = require('./bot.js');
app.post('/api/bot', async (req, res) => {
    // Diagn├│stico r├ípido
    if (!process.env.TELEGRAM_TOKEN) {
        console.error("Γ¥î ERROR: TELEGRAM_TOKEN no configurado en Vercel.");
        return res.status(200).send("Bot Desactivado: Falta Token"); 
    }
    
    if (req.body && Object.keys(req.body).length > 0) {
        console.log("≡ƒô⌐ Webhook recibido");

        // [ESTRATEGIA SERVERLESS]: Esperar a que termine todo el c├│digo as├¡ncrono
        // ANTES de enviarle el 200 a Telegram. As├¡ Vercel no corta el proceso.
        if (req.body.message && bot.procesarMensajeSync) {
            try {
                await bot.procesarMensajeSync(req.body.message);
            } catch (err) {
                console.error("Γ¥î Error grave en procesarMensajeSync:", err);
            }
        } else {
    ...
    // AHORA S├ì, decirle a Telegram y a Vercel que concluimos
    res.sendStatus(200);
});
console.log("≡ƒñû Webhook de Telegram listo en /api/bot (V3)");
...
// ≡ƒîƒ FUNCI├ôN MAESTRA PARA ALERTAS
...
// ΓÅ▒∩╕Å FUNCI├ôN PARA FORZAR LA HORA CORRECTA (UTC-5)
function obtenerHoraColombia() {
    const ahora = new Date();
    // Forzamos la zona horaria de Bogot├í (UTC-5)
...
// ≡ƒò╡∩╕Å FUNCI├ôN HELPER DE AUDITOR├ìA (No bloqueante)
async function registrarAuditoria(req, modulo, accion, detalles = {}) {
    try {
        ...
        console.error('ΓÜá∩╕Å Error silencioso al registrar auditor├¡a (┬┐Falta tabla?):', e.message);
    }
}
...
app.post('/login', async (req, res) => {
    ...
    // Primero buscar si el usuario existe (Sin filtrar clave aqu├¡ porque ahora es un Hash)
    ...
    // Comparar la contrase├▒a tipeada con el Hash de Supabase
    const passwordMatch = await bcrypt.compare(password, resultado.password);
    
    if (!passwordMatch) return res.send(mostrarAlerta('Acceso Denegado', 'La contrase├▒a es incorrecta.', 'error', '/'));
...
// ---------------- M├ôDULO PR├ëSTAMOS FABRIQUINES ----------------
...
app.post('/abonar_prestamo/:id', async (req, res) => {
    ...
    if (!prestamo) return res.send(mostrarAlerta('Error', 'Pr├⌐stamo no encontrado.', 'error'));
...
app.get('/inventario', async (req, res) => {
    ...
    // Lotes Semanales: Extraer la ├║ltima semana registrada para el tracker
...
app.post('/restar_inventario', async (req, res) => {
    ...
    if (existente && existente.cantidad >= cantNum) {
        ...
        await supabase.from('movimientos').insert([{
            ...
            cantidad: cantNum,
            usuario: req.session.usuario || 'Admin',
            descripcion: descripcion_salida || 'Salida a producci├│n / Despacho'
        }]);
    }
    res.redirect('/movimientos');
});
...
// ============================================================================
// ≡ƒöÑ RUTAS DEL SISTEMA V1.5 - MEJORAS DE DESPACHO Y MATERIA PRIMA ≡ƒöÑ
// ============================================================================
app.get('/despacho', async (req, res) => {
    ...
    // ΓöÇΓöÇ RESUMEN DE MATERIAL PENDIENTE POR ENTREGAR (Facturas 'pendiente' V2.9.3) ΓöÇΓöÇ
    let resumenSemanal = { 
    ...
    try {
        ...
        if (facturasPend) {
            ...
            facturasPend.forEach(f => {
                ...
                // Conteo de cestas (Unificaci├│n V2.9.3)
...
app.post('/despachar_tarea', async (req, res) => {
    ...
    if (fisicoEntregado <= 0) return res.send(mostrarAlerta('Error L├│gico', 'La entrega debe ser mayor a cero', 'warning'));
    
    // Kg base calculados autom├íticamente para la meta propuesta
    ...
    // Los cambios en inventario y deudas se har├ín en /confirmar_entrega_bodega
    let despachoId = null;
    try {
        const { data: dReg } = await supabase.from('despachos_registro').insert([{
            ...
            capa_kg:       capaKgBase,   // Almacenamos lo te├│rico para comparar luego
...
// --- IMPRIMIR HOJA DE DESPACHO (nueva versi├│n V2.8 con firmas) ---
app.get('/imprimir_despacho/:empleado_id', (req, res) => {
    ...
    const despacho = {
        ...
        meta_tabacos: d.meta,
        color_cesta:  d.params?.color_cesta || 'ΓÇö',
    ...
    res.render('hoja_despacho', {
        ...
        despacho,
        sacos: [] // En pr├│xima fase: sacos de picadura asignados al despacho
    });
});

// ============================================================================
// ≡ƒöÑ M├ôDULO V1.8 P2P: RECEPCI├ôN ACUMULATIVA (L-S) Y SUBPRODUCTOS ≡ƒöÑ
// ============================================================================
app.get('/recepcion_diaria', async (req, res) => {
    ...
    // Traer pr├⌐stamos activos para mostrar en la tabla
    let prestamosActivos = [];
    try {
        const { data: p } = await supabase.from('prestamos_fabriquines').select('*').eq('estado', 'activo');
        prestamosActivos = p || [];
    } catch(e) { /* tabla a├║n no migrada */ }
    ...
    // Traer historial de movimientos de pr├⌐stamos para el modal de historial
...
// --- REGISTRAR MOVIMIENTO DE PR├ëSTAMO (cualquier d├¡a, desde recepci├│n) ---
app.post('/registrar_movimiento_prestamo', async (req, res) => {
    ...
    if (!empleado_id || montoNum <= 0) return res.json({ ok: false, msg: 'Datos inv├ílidos' });
    ...
    try {
        if (tipo === 'abono') {
            ...
            // Es un pr├⌐stamo nuevo o adelanto: buscar si hay activo y sumarlo, o crear nuevo
            const { data: prestActivo } = await supabase.from('prestamos_fabriquines')
                .select('*').eq('empleado_id', empleado_id).eq('estado', 'activo').single();

            if (prestActivo) {
                // Sumar al pr├⌐stamo existente
        ...
        console.error('Error movimiento pr├⌐stamo:', e);
        return res.json({ ok: false, msg: e.message });
    }
});

// --- REGISTRAR MATERIAL EXTRA ENTRE SEMANA ---
// Cuando el fabriqu├¡n viene y pide material adicional (capa mala, faltante, etc.)
// Se suma al saldo negativo que se descuenta gradualmente en pr├│ximos despachos.
app.post('/registrar_material_extra', async (req, res) => {
    ...
    try {
        ...
        // 2. Actualizar el saldo de material (restar kg extra ΓåÆ m├ís negativo = m├ís deuda)
        ...
        } catch(e) { /* No falla si inventario no tiene esa categor├¡a */ }
...
// --- VER HISTORIAL DE PR├ëSTAMOS POR EMPLEADO (JSON) ---
...
app.post('/recepcion_diaria_guardar', async (req, res) => {
    ...
    // Funci├│n Transaccional "En Vivo" (Inyecta a DB y Crea Kardex)
    async function difStock(materialNom, categ, dif, tipoDesc) {
        if (dif === 0) return;
        const nombreCorto = emp ? emp.nombre.split(' ').slice(0, 2).join(' ') : 'Fabriqu├¡n';
    ...
    // Buscamos el ├║ltimo despacho de este fabriqu├¡n para saber qu├⌐ color de cesta se prest├│
    ...
    // --- ABONO AL PR├ëSTAMO (si se ingres├│) ---
    ...
    if (prestamoId && abonoMonto > 0) {
        ...
        } catch(e) { /* tabla prestamos a├║n no migrada */ }
    }
...
app.post('/liquidar_semana/:id', async (req, res) => {
    ...
    //     Si quedan N tabacos sin entregar, el fabriqu├¡n tiene en casa
    //     el material correspondiente a esos N tabacos. Lo guardamos
    //     para que el pr├│ximo despacho lo muestre como "Saldo Anterior".
    //     ADEM├üS sumamos cualquier deuda extra de material que ya exist├¡a
    //     (saldo_capa_kg negativo = le dieron material extra que a├║n debe)
    ...
    // Material que corresponde a los tabacos PENDIENTES (en casa del fabriqu├¡n)
    ...
    // (si saldoMaterialExtra es negativo = le dieron m├ís material del que correspond├¡a)
    ...
    // REDIRECCI├ôN DIRECTA A NOMINA
    res.redirect('/nomina');
});

// --- NUEVO M├ôDULO BIG DATA: HISTORIAL DE ENTREGAS ---
app.get('/entregas_historicas', async (req, res) => {
    if (!req.session.rol || req.session.rol !== 'admin') return res.redirect('/');
    
    // Traemos TODAS las hojas, cerradas y pendientes, en orden de creaci├│n inversa (M├ís nuevas arriba)
...
app.get('/maquinas', async (req, res) => {
    ...
    // Traemos datos de m├íquinas y de mantenimientos
    ...
    // Procesamos la l├│gica de Alertas y Costos
    const maquinasProcesadas = (maquinas || []).map(m => {
        // Encontrar mantenimientos de esta m├íquina
...
app.post('/agregar_maquina', async (req, res) => {
    ...
    if (error) {
        console.error("Γ¥î ERROR DE SUPABASE AL GUARDAR M├üQUINA:", error);
...
// --- EDITAR M├üQUINA Y FORZAR HIST├ôRICO DE MTTO ---
app.post('/editar_maquina/:id', async (req, res) => {
    ...
    // Si el administrador le meti├│ una fecha manual hist├│rica, la inyectamos para apagar alertas rojas
    ...
    if (error) {
        console.error("Γ¥î ERROR AL EDITAR M├üQUINA:", error);
    }

    res.redirect('/maquinas');
});

// --- FICHA T├ëCNICA DIGITAL (CV DE LA M├üQUINA) --- alias por ID directo
...
async function fichaHandler(req, res) {
    ...
    if (!maquina) return res.send("<h2>Γ¥î M├íquina no encontrada en el sistema.</h2>");
...
app.post('/agregar_mantenimiento', async (req, res) => {
    ...
    // SI EL MTTO ES PREVENTIVO, REINICIAMOS EL CONTADOR DE LA M├üQUINA
...
if (process.env.NODE_ENV !== 'production') {
    app.listen(PORT, () => console.log(`≡ƒÉ╛ Servidor Gato Negro corriendo en http://localhost:${PORT}`));
}

// --- GENERADOR DE C├ôDIGOS QR PARA M├üQUINAS ---
app.get('/maquina/:id/qr', async (req, res) => {
    const maquinaId = req.params.id;
    
    // 1. Detectamos autom├íticamente si estamos en Vercel (Production) o en Localhost
    const baseURL = process.env.NODE_ENV === 'production'
        ? 'https://gato-negro.vercel.app' // URL Real (Vercel)
        : `http://localhost:${PORT}`;     // URL Local (Tu PC)

    // 2. Definimos a qu├⌐ URL va a apuntar el QR (A la futura ficha t├⌐cnica)
    const urlDeLaFicha = `${baseURL}/maquinas/ficha/${maquinaId}`;

    try {
        // 3. Generamos el c├│digo QR como un Buffer de imagen PNG
        const qrBuffer = await QRCode.toBuffer(urlDeLaFicha, {
            type: 'png',
            margin: 1,
            width: 250 // Tama├▒o de la imagen
        });
...
// ==================== M├ôDULO PICADURA PROCESADA (V2.7) ====================

// --- DIVIDIR SACO EN MENORES ---
// Toma un saco disponible, lo reemplaza por sacos de menor tama├▒o
app.post('/dividir_saco', async (req, res) => {
    ...
    try {
        ...
        // 3. Obtener el n├║mero m├ís alto actual de ese lote para continuar numeraci├│n
        ...
        // Sobrante si no se distribuy├│ todo el peso
...
app.get(['/picadura', '/bodega'], async (req, res) => {
    ...
    try {
        ...
        // 2. Sacos disponibles (Orden ASC por n├║mero para Gregorio V2.11)
...
// Crea el lote y genera autom├íticamente los sacos numerados desde #1
    ...
    try {
        ...
        if (inv) {
            ...
            if (raw) {
                ...
                console.log(`Γ£à Descontado ${kgEnt}kg de Picadura Bruta (${raw.material}).`);
            }
            ...
            for (let [nombre, cant] of Object.entries(conteo)) {
                // Buscamos ignorando espacios y may├║sculas para evitar duplicados
        ...
        console.log(`≡ƒÜÇ Lote #${lote.id} finalizado en paralelo. Sincronismo total.`);
...
// --- ENTREGAR SACOS A FABRIQU├ìN ---
// --- ENTREGAR SACOS A FABRIQU├ìN (FLUJO UNIFICADO V2.11) ---
app.post('/entregar_sacos', async (req, res) => {
    // Verificaci├│n de sesi├│n flexible para evitar "error de conexi├│n" (V2.11)
    if (!req.session.usuario) {
        return res.status(401).json({ ok: false, msg: 'Sesi├│n expirada. Por favor reloguea.' });
    }
    ...
    try {
        ...
        // 2. Informaci├│n en paralelo para ganar velocidad (V2.15)
        ...
        if (!despacho) return res.json({ ok: false, msg: 'No se encontr├│ una factura vigente para despachar.' });
        ...
        // 3. ACTUALIZACI├ôN DE INVENTARIO Y SACOS EN PARALELO (TURBO V2.15)
        const promesas = [];
        ...
        // C. Descontar Sacas Espec├¡ficas - NORMALIZADO V3.3
        const conteoSacas = {};
        ...
        // 4. Finanzas: Pr├⌐stamos y Suministros
        ...
        // 7. Activar Recepci├│n Diaria y Cerrar Factura
        ...
        return res.json({ ok: true, msg: 'Entrega unificada completada con ├⌐xito.' });
    } catch(e) {
...
// ==================== FASE 5: N├ôMINA Y FACTURACI├ôN ====================

// --- DASHBOARD ANAL├ìTICO Y FINANCIERO (V2.3) ---
app.get('/analitica', async (req, res) => {
    if (!req.session.rol || req.session.rol !== 'admin') return res.redirect('/');

    // 1. Obtener gastos de N├│mina (Pagos completados a la mano obrera)
    ...
    // 4. Agrupar la s├íbana gigante de datos de Movimientos para Chart.js
...
// --- PANEL DEL FABRIQU├ìN (Cierre Diario) ---
app.get('/cierre_diario', async (req, res) => {
    ...
    // Obtener producci├│n de la semana (estado PENDIENTE)
    const { data: produccion } = await supabase.from('produccion_fabriquines')
    ...
    // Produccion manual del fabriquin fue removida. El Admin lo carga autom├ítico desde /recepcion.

// --- PANEL DE N├ôMINA (ADMINISTRADOR) ---
app.get('/nomina', async (req, res) => {
    ...
    // Nuevos arreglos Listos Para Imprimir V1.8 (Administraci├│n Central)
...
// --- CONFIRMAR ENTREGA F├ìSICA Y CREAR EN RECEPCION DIARIA ---
app.post('/confirmar_entrega_bodega', async (req, res) => {
    ...
    try {
        ...
        if (!despacho || despacho.estado !== 'pendiente') {
            return res.send(mostrarAlerta('Error', 'Orden no v├ílida o ya procesada.', 'error'));
        }
        ...
        // El nuevo saldo de material es: saldo_ant + te├│rico_esperado - real_entregado
        // Si el real es mayor al te├│rico, el saldo baja (debe material).
        ...
        return res.send(mostrarAlerta('Material Entregado', `Se ha registrado la salida f├¡sica y activado a ${empleado.nombre} para esta semana.`, 'success', '/facturas'));
...
app.get('/reimprimir_factura/:id', async (req, res) => {
    ...
    try {
        ...
        res.render('hoja_despacho', {
            empleado: { id: d.empleado_id, nombre: emp.nombre || 'ΓÇö', codigo: emp.codigo || 'ΓÇö' },
            despacho: {
                id:           d.id,
...
app.get('/imprimir_nomina_v18/:id', async (req, res) => {
    ...
    //   - tipo = 'descuento_sueldo' (Gregorio decidi├│ descontarlo del pago)
    // Los pr├⌐stamos dados (tipo='prestamo') NO se descuentan autom├íticamente del sueldo.
    let descuentos = 0;
...
// NUEVO: Abonos parciales de deuda y rezagos f├¡sicos
app.post('/abonar_deuda/:id', async (req, res) => {
    ...
    if (isNaN(abonoNum) || abonoNum <= 0) return res.send(mostrarAlerta('Error', 'Monto de abono inv├ílido.', 'error', '/nomina'));
    ...
    // 2. Si el abono fue en "rezago" (tabacos f├¡sicos), inyectarlo al inventario
    if (tipo_abono === 'rezago') {
        const tiempo = obtenerHoraColombia();
        // Buscar el material "Rezago" o "Tabacos Malos" o "Recorte"
        // Si no existe, usamos "Recorte" que es el material gen├⌐rico de merma
        const { data: inv } = await supabase.from('inventario').select('*');
        if (inv) {
...
// --- M├ôDULO 8: ├üREA DE ANILLADORES (TAREAS DIGITALES) ---
...
app.post('/anilladores/crear_tarea', async (req, res) => {
    ...
    if (!anilladorInfo) return res.send(mostrarAlerta('Error', 'Anillador Inv├ílido', 'error', '/anilladores'));
...
app.post('/anilladores/validar_tarea/:id', async (req, res) => {
    ...
    // Un simple movimiento de transformaci├│n usando 1250 tabacos por cesta (est├índar Gato Negro)
    ...
    if (invNormal && invAnillado) {
        ...
        await supabase.from('movimientos').insert([{
            fecha: tiempo.fecha, hora: tiempo.hora, tipo_movimiento: 'TRANSFORMACI├ôN',
            material: 'Tabacos Anillados', cantidad: cantTabacos, usuario: req.session.usuario || 'Admin',
            descripcion: `Validaci├│n Tarea Anillador: Conversi├│n autom├ítica de ${cantNum} Cestas a Anillados.`
        }]);
    }
...
// --- M├ôDULO 7: DESPACHOS DE ANILLADO Y EMPAQUE ---
...
// --- M├ôDULO 6: DASHBOARD ANAL├ìTICO ---
app.get('/analitica', async (req, res) => {
    if (!req.session.rol || req.session.rol !== 'admin') return res.redirect('/');
    ...
    // Producci├│n tradicional (Entradas de Bodega) para mantener c├│digo viejo vivo
    ...
    if (movEntradas) {
        movEntradas.forEach(m => {
            ...
            // Si es un tabaco gen├⌐rico o "normal", va a la primera
    ...
    // 3. Egresos (N├│mina Pagada y Pendiente)
...
// ============================================================================
// ≡ƒî» M├ôDULO ENVOLVEDORAS V2.5
// ============================================================================
app.get('/envolvedoras', async (req, res) => {
    if (!req.session.rol || req.session.rol !== 'admin') return res.redirect('/');
    // Empleados del tipo envolvedora (usamos misma tabla, filtrar por ├írea si se agrega campo)
    const { data: empleados } = await supabase.from('empleados_fabriquines').select('*').order('codigo');
...
app.post('/liquidar_envolvedora/:id', async (req, res) => {
    ...
    res.send(mostrarAlerta('Γ£à Liquidada', `Envolvedora liquidada. Total a cobrar: $${pago.toLocaleString('es-CO')} COP por ${totalCestasOut} cestas.`, 'success'));
});
